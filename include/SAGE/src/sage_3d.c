...
BOOL SAGE_Init3DModule(VOID)
{
  ...
  // Maggie3D is linked statically in this GCC build.
  Maggie3DBase = NULL;
...
#else
  SAGE_InfoLog("Warp3D driver display is not available in this GCC build");
#endif
}
...
BOOL SAGE_AllocateM3DContext(SAGE_3DDevice *device, struct Window *window)
{
  ...
  SAFE(if (window == NULL || window->RPort == NULL || window->RPort->BitMap == NULL) {
    SAGE_SetError(SERR_NO_WINDOW);
    return FALSE;
  })
  device->maggie3d_error = M3D_SUCCESS;
  device->m3d_context = M3D_CreateContext(&(device->maggie3d_error), window->RPort->BitMap);
  if (device->m3d_context == NULL) {
    SAGE_SetError(SERR_NO_3DCONTEXT);
    return FALSE;
  }
  if (!device->m3d_context->maggie_available) {
    device->maggie3d_error = M3D_NOMAGGIE;
    M3D_DestroyContext(device->m3d_context);
    device->m3d_context = NULL;
