...
SAGE_Picture *SAGE_LoadPicture(STRPTR file_name)
{
  ...
  struct TagItem new_picture_tags[] = {
    { PDTA_Remap, FALSE },
    { DTA_GroupID, GID_PICTURE },
    { PDTA_DestMode, PMODE_V43 },
    { TAG_DONE, 0 }
  };
  struct TagItem bitmap_header_tags[] = {
    { PDTA_BitMapHeader, (ULONG)&bmhd },
    { TAG_DONE, 0 }
  };
  struct TagItem palette_tags[] = {
    { PDTA_CRegs, (ULONG)&palette },
    { TAG_DONE, 0 }
  };
  struct TagItem bitmap_tags[] = {
  ...
  if (object = NewDTObjectA(file_name, new_picture_tags)) {
    GetDTAttrsA(object, bitmap_header_tags);
    if (bmhd == NULL) {
    ...
    if (DoMethodA(object, (Msg)&layout)) {
      ...
      if (bmhd->bmh_Depth == SBMP_DEPTH8) {
        ...
        GetDTAttrsA(object, palette_tags);
        if (palette == NULL) {
          SAGE_SetError(SERR_FILEFORMAT);
          DisposeDTObject(object);
      ...
      GetDTAttrsA(object, bitmap_tags);
      if (bitmap == NULL) {
        SAGE_SetError(SERR_FILEFORMAT);
        DisposeDTObject(object);
        SAGE_ReleasePicture(picture);
      ...
      if (GetCyberMapAttr(bitmap, CYBRMATTR_ISCYBERGFX)) {
        ...
        // Get the bitmap base address
        {
          struct TagItem lock_tags[] = {
            ...
            { LBMI_BYTESPERROW, (ULONG)&bm_bpr },
            { TAG_DONE, 0 }
          };
          bm_handle = LockBitMapTagList(bitmap, lock_tags);
        }
        if (bm_handle != NULL) {
