 ...
 */
BOOL SAGE_SetupScreenBuffer(struct Screen *custom_screen, SAGE_ScreenBuffer *scr_buf)
{
  SD(SAGE_DebugLog("Setup screen buffer");)
  ...
  if (scr_buf->front_buffer != NULL && scr_buf->back_buffer != NULL && scr_buf->wait_buffer != NULL) {
    ...
    SD(SAGE_DebugLog("<SAGE_SetupScreenBuffer> Wait screenbuffer bitmap");)
    SD(SAGE_DumpSystemBitmap(scr_buf->wait_buffer->sb_BitMap);)
...
BOOL SAGE_OpenScreen(LONG width, LONG height, LONG depth, LONGBITS flags)
{
  ...
  if (!SAGE_SetupScreenBuffer(screen->system_screen, &(screen->screen_buffer))) {
    SAGE_CloseScreen();
  ...
  if (screen->flags & SSCR_INDIRECT) {
    ...
    if (GetCyberMapAttr(bitmap, CYBRMATTR_ISCYBERGFX)) {
      struct TagItem lock_tags[] = {
        { LBMI_BASEADDRESS, (ULONG)&bmp_adr },
        { TAG_DONE, 0 }
      };
      cgx_handle = LockBitMapTagList(bitmap, lock_tags);
      if (cgx_handle != NULL) {
...
/**
 * Get the screen wait buffer address
 * 
 * @return Wait bitmap
 */
SAGE_Bitmap *SAGE_GetWaitBitmap()
{
  SAGE_Screen *screen;
  ...
  return screen->wait_bitmap;
}

/**
 * Get the screen system wait bitmap
 * 
 * @return System wait bitmap
 */
struct BitMap *SAGE_GetSystemWaitBitmap(VOID)
{
  SAGE_Screen *screen;
  ...
  return screen->screen_buffer.wait_buffer->sb_BitMap;
}
...
/**
 * Print a formatted text on a screen position
 * 
 * @param posx   Text X position
 * @param posy   Text Y position
 * @param format Text string
 * @param args   Variable list of arguments
 * 
 * @return Operation success
 */
BOOL SAGE_PrintFText(UWORD posx, UWORD posy, char *format, ...)
{
  SAGE_Screen *screen;
  va_list args;
  ...
  va_end(args);
  Text(&(screen->screen_buffer.work_rastport), TextBuffer, strlen(TextBuffer));
  return TRUE;
}
...
/**
 * Print a formatted text directly on a screen position
 * 
 * @param posx   Text X position
 * @param posy   Text Y position
 * @param format Text string
 ...
 * @return Operation success
 */
BOOL SAGE_PrintDirectFText(UWORD posx, UWORD posy, char *format, ...)
{
  SAGE_Screen *screen;
  va_list args;
  ...
  vsprintf(TextBuffer, format, args);
  va_end(args);
  Text(&(screen->system_screen->RastPort), TextBuffer, strlen(TextBuffer));
  return TRUE;
}
